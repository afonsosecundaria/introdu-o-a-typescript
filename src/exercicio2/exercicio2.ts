...
class Familia{
    renda: number
    filhos: number
    constructor(renda: number, filhos: number){
        this.renda = renda
        this.filhos = filhos
    }
}
...
class IntervaloPrimos {
  private verificador: VerificadorPrimo;

  constructor() {
    this.verificador = new VerificadorPrimo();
  }

  contarPrimos(inicio: number, fim: number): number {
    let contador = 0;
...
console.log(
  `Entre ${inicio} e ${fim} existem ${quantidade} números primos`
);